...
import { PROTOTYPES_PART_1 } from '../../data/slidesData';
...
export const Slide5BayanihanFramework: React.FC<Slide5Props> = ({ meta, totalSlides, onNext }) => {
  const [selectedProto, setSelectedProto] = useState<PrototypeWorkstream | null>(null);

  const currentIndex = selectedProto
    ? PROTOTYPES_PART_1.findIndex((p) => p.id === selectedProto.id)
    : -1;

  const hasPrev = currentIndex > 0;
  const hasNext = currentIndex >= 0 && currentIndex < PROTOTYPES_PART_1.length - 1;

  const handlePrevProto = () => {
    if (hasPrev) {
      setSelectedProto(PROTOTYPES_PART_1[currentIndex - 1]);
    }
  };
  ...
  const prevProto = hasPrev ? PROTOTYPES_PART_1[currentIndex - 1] : null;
  const nextProto = hasNext ? PROTOTYPES_PART_1[currentIndex + 1] : null;

  const getProtoIcon = (iconName: string) => {
    switch (iconName) {
      case 'Compass':
        return <Compass className="w-5 h-5 text-[#0055FF]" />;
      case 'Sprout':
        return <Sprout className="w-5 h-5 text-[#059669]" />;
      case 'UsersRound':
      default:
        return <UsersRound className="w-5 h-5 text-[#0284C7]" />;
    }
  };
    ...
    <div className="h-full flex flex-col justify-between p-3 sm:p-4 md:p-5 select-none relative overflow-y-auto bg-[#FFFFFF]">
        ...
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2 mb-0.5">
            ...
            <div className="immersive-chip text-[8.5px] sm:text-[9px] text-[#0055FF] bg-[#EBF3FF] border-[#0055FF]/30 shrink-0 self-start sm:self-auto">
              Prototypes 01 &ndash; 03 of 06
            </div>
          </div>
          <p className="text-xs sm:text-[13px] text-[#555555] leading-normal font-body">
            Prototyping spatial intelligence, regenerative multi-use sites, and community equity architectures. Click any prototype to open the technical deep-dive dialog.
          </p>
        </div>

        {/* 3 Interactive Prototype Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3.5 my-auto py-1 sm:py-2">
          {PROTOTYPES_PART_1.map((proto, idx) => (
            <motion.div
              key={proto.id}
        ...
        <div className="p-2.5 rounded-[3px] bg-[#F8FAFC] border border-[#E2E8F0] flex items-center justify-between text-xs sm:text-[13px] text-[#555555]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#1E2A38] text-xs sm:text-[13px] font-body">
              <strong className="text-[#0055FF] futura font-bold">Part 1 Focus:</strong> Upstream spatial screening with NAMRIA, multi-use agro-marine hubs with LGUs, and democratic community equity co-ownership.
            </span>
          </div>
      ...
      <footer className="flex items-center justify-between border-t border-[#E2E8F0] pt-2 text-xs mt-1 shrink-0">
        <span className="text-[#555555] font-mono-code text-[11px] sm:text-[12.5px] truncate max-w-[65%] sm:max-w-[70%]">
          NAMRIA GeoPH &bull; LGU Dual-Use Hubs &bull; Bayanihan Co-Ownership Trusts
        </span>
        <button
          onClick={onNext}
        ...
        >
          <span>Next: Prototypes (Part 2)</span>
      ...
      <SlideDetailModal
        ...
        counterText={currentIndex >= 0 ? `Prototype ${currentIndex + 1} of ${PROTOTYPES_PART_1.length}` : undefined}
      >
        {selectedProto && (
          <div className="space-y-3">
            ...
            {selectedProto.technicalMechanism && (
              <div className="p-3.5 rounded-[3px] bg-[#FFFFFF] border border-[#E2E8F0]">
                <div className="font-mono-code text-xs font-bold text-[#0055FF] uppercase mb-1 flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5 text-[#0055FF]" /> Technical Mechanism & Architecture
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              ...
              <div className="p-3.5 rounded-[3px] bg-[#FFFFFF] border border-[#E2E8F0]">
                <div className="font-mono-code text-xs font-bold text-[#0055FF] uppercase mb-1.5 flex items-center gap-1">
                  <Users className="w-3.5 h-3.5 text-[#0055FF]" /> Target Stakeholders & Anchors
                </div>
